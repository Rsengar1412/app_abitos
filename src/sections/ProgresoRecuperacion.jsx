      ...
      <div className="relative flex flex-col gap-0 border-l-2 border-bg-primary pl-6 ml-2">
        {MILESTONES.map((milestone) => {
          ...
          return (
            ...
            >
              <div
                className={`absolute left-[-9px] w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-sm border-2 transition-all bg-bg-primary border-text-secondary text-text-secondary ${unlocked ? 'bg-success border-success text-white' : ''}`}
              >
                {unlocked ? <Check size={16} /> : <Lock size={14} />}
              </div>
              <div className="flex-1 pt-0.5">
                <div className="text-sm font-semibold text-text-secondary mb-1 uppercase tracking-wide">
                  Día {milestone.days}
                </div>
                <h4
                  className="text-lg font-bold mb-2 text-text-primary"
                  style={{ color: unlocked ? milestone.color : 'inherit' }}
                >
                  {milestone.title}
                </h4>
                <ul className="text-sm text-text-secondary leading-relaxed m-0 list-disc ml-4">
                  {milestone.benefits.map((benefit) => (
                    <li key={`${milestone.days}-${benefit}`} className="mb-1">
                      {benefit}
                    </li>
                  ))}
                </ul>
