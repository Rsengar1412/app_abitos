...
const Logros = ({ habits = EMPTY_HABITS, currentMaxDays = 0 }) => {
  const nextAchievement =
    ACHIEVEMENTS.find((a) => a.days > currentMaxDays) || ACHIEVEMENTS[ACHIEVEMENTS.length - 1];
  const lastAchDays = ACHIEVEMENTS.filter((a) => a.days <= currentMaxDays).pop()?.days || 0;
  const progressPercent = Math.min(
    100,
    Math.max(0, ((currentMaxDays - lastAchDays) / (nextAchievement.days - lastAchDays)) * 100)
  );
    ...
    <div className="p-6 mt-8 sm:p-4">
      ...
      <div className="flex justify-center gap-4 mb-8 flex-wrap">
        {habits.map((h) => (
          ...
          >
            <span className="text-[0.65rem] text-text-secondary uppercase tracking-wider mb-0.5">
              {h.name}
            </span>
          ...
          return (
            ...
            >
              <div className="relative text-4xl mb-3 inline-block">
                ...
                {isUnlocked && (
                  <CheckCircle2
                    className="absolute -bottom-0.5 -right-1 bg-success text-white rounded-full p-0.5 border-2 border-bg-secondary"
                    size={16}
                  />
                )}
              </div>
